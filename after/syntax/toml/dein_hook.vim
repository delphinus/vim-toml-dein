...
syn region tomlDeinScriptOption start=/^\s*\zs\%(if\|on_if\|hook_\%(add\|done_update\|post_source\|post_update\|source\)\)\s*=\s*\z('''\|"""\|'\(''\)\@!\|"\(""\)\@!\)/ end=/\z1/ contains=tomlKey,tomlDeinScript keepend
